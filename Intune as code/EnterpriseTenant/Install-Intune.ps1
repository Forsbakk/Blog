$tenantname = '@M365x720586.onmicrosoft.com'

$User = "admin" + $tenantname #Change this to your username
$JSONFilesToInstall = Get-ChildItem $PSScriptRoot | Where-Object { $_.Extension -eq ".json" } #Location of JSONS
$UserGroup = "All Employees" #User group to deploy production material
...
$AdminUsersToCreate = @( #Admin-users that will be created
    @{
        Name = "Jonas Admin"
    }
)
$NormalUsersToCreate = @( #Normal users to be created
    @{
        Name = "Jonas Forsbakk"
    }
)
...
#Clean up App protection policies that comes with a demo tenant
try {
    $uri = "https://graph.microsoft.com/Beta/deviceAppManagement/managedAppPolicies"
    $AppPolicies = (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).Value
}
...
ForEach ($pol in $AppPolicies) {
    try {
        $uri = "https://graph.microsoft.com/Beta/deviceAppManagement/managedAppPolicies/$($pol.id)"
        Invoke-RestMethod -Uri $uri -Headers $authToken -Method Delete
    }
...
#Clean up Client apps that comes with a demo tenant
#Will produce errors when trying to delete undeletable apps
try {
    $uri = "https://graph.microsoft.com/Beta/deviceAppManagement/mobileApps"
    $Apps = (Invoke-RestMethod -Uri $uri -Headers $authToken -Method Get).Value
}
...
foreach ($User in $AdminUsersToCreate) {
    ...
    $userPrincipalName = $givenName.Substring(0, 3) + $surname.Substring(0, 3) + $tenantname
